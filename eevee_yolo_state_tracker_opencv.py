...
from eevee_poke_state_common import *
...
#    def __init__(self, trainer_name='Lou Rui', network_style=NETWORK_STYLE.RANDO_BOT):
        WildState.__init__(self, trainer_name, network_style)
...
#        self.navigation_actions = [NavigationAction.STANDBY_COUNT_ACTIVE_POKEMON]
    ...
    def process_frame(self, image, labels_boxes, override_sequence=None):
        ...
        state = eevee_update_state(labels_boxes)
        ...
        next_battle_selectable = eevee_update_selectables(self.battle_state, labels_boxes)
        mini_menu = self.sequence == SEQUENCE_STATE.AWAITING_SWITCH
        next_battle_sub_selectable = eevee_update_sub_selectables(self.battle_state, next_battle_selectable, labels_boxes, mini_menu)
...
#        print('next_battle_sub_selectable:', next_battle_sub_selectable)
        ...
        #TODO Update to check for cannot switch along with navigation
        if False and self.battle_state == BATTLE_STATES.TEAM_MENU_SWITCH_INFO or 'switch_error_message' in labels_boxes:
        ...
        if done:
            ...
            if self.current_navigation_action == NavigationAction.STANDBY_COUNT_ACTIVE_POKEMON:
                ...
                for i in range(self.active_pokemon_count):
                    ...
                    self.navigation_actions.insert(0, NavigationAction.STANDBY_CHECK_NEXT_ACTIVE_PLAYER_A)
                    self.navigation_actions.insert(0, NavigationAction.STANDBY_CHECK_NEXT_ACTIVE_PLAYER_B)
...
#                self.futures.append(future)
                ...
                future = self.pool.apply_async(self.extract_pokemon_info_page_1, [pkmn_name, name_rect, item_rect, element_1_rect, element_2_rect, hp_rect, sp_atk_rect, atk_rect, sp_def_rect, def_rect, speed_rect, attack_slot_1_rect, attack_slot_2_rect, attack_slot_3_rect, attack_slot_4_rect], callback=self.construct_pokemon_info_page_1)
                self.futures.append(future)
                done = True
                wait = 0.43
        ...
        if nav_action in [NavigationAction.STANDBY_CHECK_NEXT_ACTIVE_PLAYER_A, NavigationAction.STANDBY_CHECK_NEXT_ACTIVE_ENEMY_A, NavigationAction.STANDBY_CHECK_NEXT_ACTIVE_PLAYER_B, NavigationAction.STANDBY_CHECK_NEXT_ACTIVE_ENEMY_B]:
...
#        position = self.get_position_for_pokemon(is_player, pkmn_name)
    ...
    @staticmethod
    def extract_pokemon_info_page_1(pkmn_name, name_rect, item_rect, element_1_rect, element_2_rect, hp_rect, sp_atk_rect, atk_rect, sp_def_rect, def_rect, speed_rect, attack_slot_1_rect, attack_slot_2_rect, attack_slot_3_rect, attack_slot_4_rect):
...
#                print(atk_data)
        ...
        return pkmn_name, name, item, element_1, element_2, hp, sp_atk, atk, sp_def, defense, speed, attacks

    def construct_pokemon_info_page_1(self, future):
        print('before future constructing page one info plus plus')
        pkmn_name, name, item, element_1, element_2, hp, sp_atk, atk, sp_def, defense, speed, attacks = future
        sample_max_moves = [('Max Overgrowth', 10), ('Max Airstream', 10), ('Max Darkness', 10), ('Max Flare', 10)]
...
def eevee_update_state(labels_boxes):
    battle_state = None
    for label in labels_boxes:
...
def eevee_update_selectables(battle_state, labels_boxes, network_match=False):
    ...
    for label in labels_boxes:
        for box in labels_boxes[label]:
            if label in battle_selectables_labels:
                print(label)
                print(box)
                battle_selectable = BATTLE_SELECTABLES.get_item_for_rect(battle_state, box)
    return battle_selectable

def eevee_update_sub_selectables(battle_state, battle_selectable, labels_boxes, mini=False):
...
def process_live_video_feed_eevee():
    import os
    ...
    #    _out = cv2.VideoWriter("test_poke.mp4", _fourcc, 20.0, (1920,1080))
        ...
        while(True):
            print('FIght')
            print(BATTLE_SELECTABLES.STANDBY_FIGHT.get_rect())
            if action is not None:
                for i in range(5):
                    cam.grab()
            action, done = None, False
...
if __name__ == '__main__':
    yolo = YOLO(**{
                "score" : 0.8,
                "gpu_num" : 0,
                "model_path": "eevee_model_data/yolov3_med-416_40000.weights",
                "config_path": "eevee_model_data/yolov3_med-416.cfg",
        #        "model_path": "eevee_model_data/yolo-obj-416-shrunk_10000.weights",
        #        "config_path": "eevee_model_data/yolo-obj-416-shrunk.cfg",
                "meta_path": "eevee_model_data/obj.data",
                "classes_path": 'eevee_model_data/obj.names',
                }
               )



    process_live_video_feed_eevee()
    i = 1/0
